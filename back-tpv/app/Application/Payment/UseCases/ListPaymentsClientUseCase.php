...
final class ListPaymentsClientUseCase
{
    ...
    public function __invoke(ListPaymentsClientCommand $command): Collection
    {
        $idPedido = $command->getIdPedido();
        $this->validateOrFail($idPedido);
        $payments = $this->paymentRepository->findByClient($idPedido);
