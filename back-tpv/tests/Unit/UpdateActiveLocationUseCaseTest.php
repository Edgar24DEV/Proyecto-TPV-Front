...
class UpdateActiveLocationUseCaseTest extends TestCase
{
    public function test_update_active_location_successfully()
    {
        // Arrange
        $command = new UpdateActiveLocationCommand(id: 1, ubicacion: "Terraza", activo: true, idRestaurante: 1);
        ...
        $locationRepo->method('updateActive')->with($command)->willReturn($updatedLocation);
        $locationService->method('showLocationInfoSimple')->with($updatedLocation)->willReturn($updatedLocation);

        // Caso de uso
        $useCase = new UpdateActiveLocationUseCase($locationRepo, $restaurantRepo, $roleRepo, $companyRepo, $locationService);
    ...
    public function test_update_active_location_fails_when_invalid_location_id()
    {
        // Arrange
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage("ID ubicacion inválido");

        $command = new UpdateActiveLocationCommand(id: -1, ubicacion: "Barra", activo: false, idRestaurante: 1); // ID inválido
    ...
    public function test_update_active_location_fails_when_location_not_belongs_to_restaurant()
    {
        // Arrange
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage("Ubicacion no pertenece al restaurante especificado");

        $command = new UpdateActiveLocationCommand(id: 1, ubicacion: "Salón", activo: true, idRestaurante: 2);
