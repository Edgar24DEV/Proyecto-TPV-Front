...
class ListProductsRestaurantUseCaseTest extends TestCase
{
    public function test_list_products_restaurant_successfully()
    {
        // Arrange
        $command = new ListProductsRestaurantCommand(idRestaurante: 1);
        ...
        $productRepo->method('find')->with($command->getIdRestaurant())->willReturn($products);
        $productService->method('showProductInfo')->with($products)->willReturn($processedProducts);

        // Caso de uso
        $useCase = new ListProductsRestaurantUseCase($restaurantRepo, $productRepo, $productService);
    ...
    public function test_list_products_restaurant_fails_when_invalid_restaurant_id()
    {
        // Arrange
