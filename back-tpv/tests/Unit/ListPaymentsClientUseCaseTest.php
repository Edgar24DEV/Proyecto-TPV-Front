...
class ListPaymentsClientUseCaseTest extends TestCase
{
    public function test_list_payments_client_successfully()
    {
        // Arrange
        $command = new ListPaymentsClientCommand(idCliente: 1);
        ...
        // Mock payment data
        $payments = [
            new Payment(id: 1, tipo: "Tarjeta", cantidad: 50.00, fecha: "2024-05-14", idPedido: 10, idCliente: 1),
            new Payment(id: 2, tipo: "Efectivo", cantidad: 20.00, fecha: "2024-05-14", idPedido: 11, idCliente: 1)
        ];

        $processedPayments = new Collection([
            new Payment(id: 1, tipo: "Tarjeta - Procesado", cantidad: 50.00, fecha: "2024-05-14", idPedido: 10, idCliente: 1),
            new Payment(id: 2, tipo: "Efectivo - Procesado", cantidad: 20.00, fecha: "2024-05-14", idPedido: 11, idCliente: 1)
        ]);

        // Expectations
        $orderRepo->method('exist')->with($command->getIdPedido())->willReturn(true);
        $paymentRepo->method('findByClient')->with($command->getIdPedido())->willReturn($payments);
        $paymentService->method('showPaymentInfo')->with($payments)->willReturn($processedPayments);

        // Caso de uso
        $useCase = new ListPaymentsClientUseCase($paymentRepo, $orderRepo, $paymentService);
    ...
    public function test_list_payments_client_fails_when_invalid_client_id()
    {
        // Arrange
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage("ID No existe");

        $command = new ListPaymentsClientCommand(idCliente: 999); // Cliente inexistente
