...
class GetRestaurantOrdersUseCaseTest extends TestCase
{
    public function test_get_restaurant_orders_successfully()
    {
        // Arrange
        $command = new GetRestaurantOrdersCommand(idRestaurante: 1);
        ...
        $orderRepo->method('getRestaurantOrders')->with($command)->willReturn($orders);
        $orderService->method('showOrderInfo')->with($orders)->willReturn($processedOrders);

        // Caso de uso
        $useCase = new GetRestaurantOrdersUseCase($orderRepo, $restaurantRepo, $orderService);
    ...
    public function test_get_restaurant_orders_fails_when_invalid_restaurant_id()
    {
        // Arrange
        $this->expectException(\Exception::class);
        $this->expectExceptionMessage("ID restaurante inválido");

        $command = new GetRestaurantOrdersCommand(idRestaurante: -1); // ID inválido
